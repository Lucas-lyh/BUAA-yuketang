...
# 加载本地HTML文件
driver.get(os.path.join(os.path.abspath("./rain-classroom"), "index.html"))
driver.implicitly_wait(3)

from tqdm import trange

print(
    "浏览器已启动！请在20秒内导航至雨课堂网站并登录，然后进入目标课程的'成绩单'页面。"
)
print("20秒倒计时开始...")
# 给用户20秒时间导航至成绩单页面
for _ in trange(20):
...
for i, item in enumerate(course_list):
    if (
        ...
        for window_handle in driver.window_handles:
            if window_handle not in pre_window_list:
        ...
        if len(video_box) > 0:
            ...
            while not done:
                ...
                is_dual = len(driver.find_elements(By.CLASS_NAME, "digital-human")) > 0
                if is_dual:
                    ...
                    start_element = start_element.find_elements(By.TAG_NAME, "div")[1]
...
speedlistBtn.click();
                        ...
                        time.sleep(0.2)
                        ActionChains(driver).move_to_element(speed_element).perform()
                        time.sleep(0.2)
